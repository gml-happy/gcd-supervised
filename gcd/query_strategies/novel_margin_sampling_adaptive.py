...
'''
our proposed method: Adaptive-Novel
sample as more novel samples as possible, with each as high entropy as possible
diversity & novelty & informativeness
'''


class NovelMarginSamplingAdaptive(Strategy):
    def __init__(self, al_dataset, original_train_loader, original_test_loader, original_unlabelled_train_loader,
        ...
        super(NovelMarginSamplingAdaptive, self).__init__(al_dataset, original_train_loader, original_test_loader, original_unlabelled_train_loader,
                 original_train_labeled_loader_ind_mapping, al_net, train_transform, test_transform,args)
    ...
    def query(self, n, current_round):
        ...
        if current_round >= self.args.adaptive_round:
            print('Novel Margin Sampling...')
            sorted_idxs = uncertainties.sort()[1]
        else:
            print('Novel Margin Sampling Reverse...')
